...
# load data ----
setwd("")
dot.comp_all <- read_excel("",sheet=1,na="NA")
head(dot.comp_all)
...
for (i in unique(dot.comp_all$site)) {
  for(j in unique(dot.comp_all$plot)) {
    ...
    rela_plot <- unique(subset_data$real_plot)
    
    subset_data <- subset_data %>%
      dplyr::select(!year) %>%
      filter(!duplicated(cbind(X_mm, Y_mm)))
    
    subset_data$species_English <- as.factor(subset_data$species_English)
    
    Ω0_500r <- data.frame()
    bands <- c(0,100,200,300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1600,1700,1800,1900)
    ...
    Ω0_500r <- replace(Ω0_500r, is.na(Ω0_500r), 0)
    
    Ω0_500r2 <- Ω0_500r %>% 
      #filter(cD != 'NA') %>%
      #filter(hD != 'NA') %>%
      filter(species_English %in% sel_species2$species_English) %>% 
      merge(sel_species2) %>%
      ...
      mutate(cΩ = rtcD/mean_cden,
             ...
             site = i, plot = j, rela_plot = rela_plot)
    
    Ω_all_site_0_500r <- bind_rows(Ω_all_site_0_500r, Ω0_500r2)
